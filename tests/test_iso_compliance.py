...
from apps.crl.core.models import BaseArtifact, ArtifactType, ArtifactStatus, ArtifactVisibility, ArtifactRelation
...
async def test_iso27001_access_control_sync_leakage():
    ...
    private_trace = BaseArtifact(
        title="Secret", type=ArtifactType.TRACE, project_id="p1",
    ...
    protected_draft = BaseArtifact(
        title="Draft", type=ArtifactType.HYPOTHESIS, project_id="p1",
    ...
    public_artifact = BaseArtifact(
        title="Public Knowledge", type=ArtifactType.OBSERVATION, project_id="p1",
...
async def test_iso42001_provenance_audit_trail():
    ...
    async with async_session() as session:
        ...
        # 1. Original
        origin = BaseArtifact(title="Origin", type=ArtifactType.HYPOTHESIS, project_id="p1")
