...
async def test_store_artifact_success():
    async with respx.mock(base_url="http://test-rae") as respx_mock:
        ...
        respx_mock.post("/memories").mock(return_value=Response(200, json={"status": "ok"}))
...
async def test_store_artifact_failure():
    async with respx.mock(base_url="http://test-rae") as respx_mock:
        ...
        client = RAEClient(base_url="http://test-rae")
        artifact = BaseArtifact(title="Test", type=ArtifactType.HYPOTHESIS, project_id="p1", description="desc")
...
async def test_query_artifacts():
    async with respx.mock(base_url="http://test-rae") as respx_mock:
        ...
        respx_mock.post("/memories/query").mock(return_value=Response(200, json={"results": mock_results}))
