...
from apps.crl.core.models import BaseArtifact, ArtifactVisibility
...
class SyncEngine:
    ...
    async def run_sync_cycle(self):
        async for session in get_session():
            ...
            query = select(BaseArtifact).where(
                BaseArtifact.visibility != ArtifactVisibility.PRIVATE
            ).where(
                (BaseArtifact.grace_period_end == None) | (BaseArtifact.grace_period_end < now)
            ).where(
                (BaseArtifact.last_synced_at == None) | (BaseArtifact.updated_at > BaseArtifact.last_synced_at)
            )
